...
use crate::keybindings::{ShortcutCommand, ShortcutRegistry};
...
impl SketchBoard {
    ...
    fn dispatch_key_shortcut_command(
        ...
        match command {
            ...
            ShortcutCommand::SaveToFile => {
                self.renderer.request_render(&[Action::SaveToFile]);
                ToolUpdateResult::Unmodified
            }
            ShortcutCommand::SaveToFileAs => {
                self.renderer.request_render(&[Action::SaveToFileAs]);
                ToolUpdateResult::Unmodified
            }
            ShortcutCommand::SaveToClipboard => {
                self.renderer.request_render(&[Action::SaveToClipboard]);
                ToolUpdateResult::Unmodified
            }
            ShortcutCommand::CopyFilepathToClipboard => {
                self.renderer
            ...
            ShortcutCommand::RunActionsOnEscape => {
                if let ToolUpdateResult::Unmodified = active_tool_result {
                    self.renderer
                        .request_render(&APP_CONFIG.read().actions_on_escape());
                }
                active_tool_result
            }
            ShortcutCommand::RunActionsOnEnter => {
                if let ToolUpdateResult::Unmodified = active_tool_result {
                    self.renderer
                        .request_render(&APP_CONFIG.read().actions_on_enter());
                }
                active_tool_result
            }
        }
...
impl Component for SketchBoard {
        ...
        gtk::Box {
            ...
            area -> FemtoVGArea {
                ...
                add_controller = gtk::EventControllerKey {
                    connect_key_pressed[sender ,im_context = model.im_context.clone(), ime_enabled = model.ime_enabled.clone()] => move |controller, key, code, modifier | {
    ...
    fn update(&mut self, msg: SketchBoardInput, sender: ComponentSender<Self>, _root: &Self::Root) {
        if !self.ime_enabled.get() && self.active_tool_type() == Tools::Text {
            self.ime_enabled.set(true);
        } else if self.ime_enabled.get() && self.active_tool_type() != Tools::Text {
