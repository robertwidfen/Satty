...
impl Drawable for SelectionOverlay {
    fn draw(
        ...
        // draw handles in inverse zoom scale to so the visual size stays constant on screen.
        let border_outset = SELECTION_BORDER_OUTSET;
        ...
        // Selection rectangle
        let border_tl = Vec2D::new(self.tl.x - border_outset, self.tl.y - border_outset);
        let border_w = w + 2.0 * border_outset;
        let border_h = h + 2.0 * border_outset;
